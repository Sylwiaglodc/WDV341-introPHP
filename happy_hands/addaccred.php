...
{
	global $inorg, $validForm, $orgErrMsg;		//Use the GLOBAL Version of the variables
	$orgErrMsg = "";								//Clearing the error message.
	if($inorg=="")
	{
		$validForm = false;					//Invalid name so the form is invalid
		$orgErrMsg = "Name is required";	//Error message for this validation
	}
}
	...
	{
		global $inyear, $validForm, $yearErrMsg;		//Use the GLOBAL Version of the variables
		$yearErrMsg = "";								//Clear the error message.
		if($inyear=="")
		{
			$validForm = false;					//Invalid name so the form is invalid
			$yearErrMsg = "Speciality is required";	//Error message for this validation
		}
	}

	function validateLevel()
	{
		global $inlevel, $validForm, $levelErrMsg;		//Use the GLOBAL Version of the variables
		$levelErrMsg = "";								//Clear the error message.
		if($inlevel=="")
		{
			$validForm = false;					//Invalid name so the form is invalid
			$levelErrMsg = "About is required";	//Error message for this validation
		}
	}
	...
	$validForm = true;					//Set form flag/switch to true.  Assumes a valid form so far
		...
		if($validForm)
		{
			...
			try {
				...
				$sql = "INSERT INTO accreds (";
				$sql .= "organization, ";
				$sql .= "yearaccred, ";
				$sql .= "level ";
				$sql .= ") VALUES (:org, :year, :level)";

				//PREPARE the SQL statement
				$stmt = $conn->prepare($sql);

				//BIND the values to the input parameters of the prepared statement
				$stmt->bindParam(':org', $inorg);
				$stmt->bindParam(':year', $inyear);
				$stmt->bindParam(':level', $inlevel);


				//EXECUTE the prepared statement
				$stmt->execute();

				$message = "ACCREDITATION HAS BEEN ADDED, THANKS!";
			}
	...
	else
	{
		...
		$message = "Please enter accredidation information on the form!";
	}
...
<meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
  ...
  <header>
             <div class="container">
			<h1 class="header">Happy Hands Daycare Insert Accredidation Section</h1>
			<h2 class="logindesc">Please use the form below to enter a new accredidation into the system. </h2>
...
<nav class="mb-1 navbar navbar-expand-lg navbar-dark">
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent-555">
    ...
    <form method="post" action="addaccred.php" name = "addaccred" id="addaccred">
      <h1>Add Accredidation</h1>
      <label for="orgName"><p><strong>Organization Name:</strong></p></label>
      <td class="error"><?php echo "$orgErrMsg"; ?></td><br>
      <input type = "text" id="orgName" name="orgName"></input><br><br>


      <label for="year"><p><strong>Accredidation Year: </strong></p></label>
      <td class="error"><?php echo "$yearErrMsg"; ?></td><br>
      <input type = "text" id="year" name="year"></input><br><br>

      <label for="level">Choose an accredidation level:</label>
      <td class="error"><?php echo "$levelErrMsg";  ?></td><br>
